...
"   boolean WorkunitExists(const varstring wuid, boolean online=true, boolean archived=false) : c,volatile,entrypoint='wsWorkunitExists'; \n"
"   dataset(WsWorkunitRecord) WorkunitList("
                                        ...
                                        ") : c,volatile,context,entrypoint='wsWorkunitList'; \n"
"  varstring WUIDonDate(unsigned4 year,unsigned4 month,unsigned4 day,unsigned4 hour, unsigned4 minute) : c,pure,entrypoint='wsWUIDonDate'; \n"
"  varstring WUIDdaysAgo(unsigned4  daysago) : c,volatile,entrypoint='wsWUIDdaysAgo'; \n"
"  dataset(WsTimeStamp) WorkunitTimeStamps(const varstring wuid) : c,volatile,context,entrypoint='wsWorkunitTimeStamps'; \n"
"  dataset(WsMessage) WorkunitMessages(const varstring wuid) : c,volatile,context,entrypoint='wsWorkunitMessages'; \n"
"  dataset(WsFileRead) WorkunitFilesRead(const varstring wuid) : c,volatile,context,entrypoint='wsWorkunitFilesRead'; \n"
"  dataset(WsFileWritten) WorkunitFilesWritten(const varstring wuid) : c,volatile,context,entrypoint='wsWorkunitFilesWritten'; \n"
"  dataset(WsTiming) WorkunitTimings(const varstring wuid) : c,volatile,context,entrypoint='wsWorkunitTimings'; \n"
