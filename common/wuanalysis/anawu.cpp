...
//The following are computed:
    ...
    PointerArrayOf<WaThread> threadFilter;
};
...
{
public:
    WuHotspotResult(WuScope * _activity, const char * _parent, bool _isRoot, double _totalPercent, double _startPercent, double _myStartPercent, double _runPercent, bool _startSignificant, bool _runSignificant);
...
= { {watOptMinInterestingTime, "minInterestingTime", 1000, wutOptValueTypeMSec},
    ...
    {watIsRoxie, "isRoxie", true, wutOptValueTypeBool},
};
...
public:
    ...
    void findHotspots(const StringArray & args);
...
WuHotspotResult::WuHotspotResult(WuScope * _activity, const char * _parent, bool _isRoot, double _totalPercent, double _startPercent, double _myStartPercent, double _runPercent, bool _startSignificant, bool _runSignificant)
: activity(_activity), parent(_parent), isRoot(_isRoot), totalPercent(_totalPercent), startPercent(_startPercent), myStartPercent(_myStartPercent), runPercent(_runPercent), startSignificant(_startSignificant), runSignificant(_runSignificant)
{
}
...
void WuHotspotResult::reportTime()
{
    ...
    printf("%s%-6s {%.9f..%.9f,%.9f..%.9f} %s(%-6s %6.2f%%,%6.2f%%,%6.2f%%) %s\n",
            ...
            prefix, parent.str(), totalPercent, startPercent, runPercent, activityKindStr(kind));
/*
...
void WorkunitAnalyser::findHotspots(const StringArray & args)
{
...
- If using a woorkunit that was generated to store the stats, then use that workunit. E.g.
...
name [R](parent  total%, start%,  mysta%   run%)
