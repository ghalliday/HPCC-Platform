...
IHqlExpression * buildDiskFileViewerEcl(const char * logicalName, IHqlExpression * record)
{
    ...
    OwnedHqlExpr dataset = createDatasetF(no_table, createConstant(logicalName), newRecord.getLink(), createValue(no_thor), NULL);
...
{
    OwnedHqlExpr dataset = createDatasetF(no_table, createConstant(logicalName), LINK(record), createValue(no_thor), NULL);
