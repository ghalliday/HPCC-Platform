...
enum FieldMatchType : unsigned
{
    ...
    match_keyedpayload= 0x200,   // A source payload field is used as a keyed field in the destination
    match_payloadinkey= 0x400,   // A source keyed field is used as a payload field in the destination (should be safe)
...
StringBuffer &describeFlags(StringBuffer &out, FieldMatchType flags)
{
    ...
    if (flags & match_keyedpayload) out.append("|keyedpayload");
    if (flags & match_payloadinkey) out.append("|payloadinkey");
...
// IDynamicTransform impl.
    ...
    {
        return (matchFlags & ~(match_link|match_virtual|match_keyedpayload|match_keyedchange|match_inifblock)) != 0;
    }
    virtual bool keyedTranslated() const override
    {
        return (matchFlags & match_keyedpayload) != 0;
    }
            ...
            else
            {
                    ...
                    else
                    {
                        ...
                        case type_dictionary:
                        {
                            ...
                            else if (info.subTrans->canTranslate())
                            {
                                ...
                                matchFlags |= (FieldMatchType)(childFlags & ~(match_keyedchange|match_keyedpayload|match_payloadinkey));
                            }
                            else
                ...
                if ((field->flags & RFTMispayloadfield) != (sourceFlags & RFTMispayloadfield))
                {
                    if (sourceFlags & RFTMispayloadfield)
                        matchFlags |= match_keyedpayload;   // Source is not keyed, but the ecl is expecting it to be
                    else
                        matchFlags |= match_payloadinkey;   // Source is keyed but ecl is not expecting it to be
                }
            }
        ...
        if (sourceRecInfo.getNumFields() > destRecInfo.getNumFields()-defaulted)
        {
            ...
            for (unsigned idx = 0; idx < sourceRecInfo.getNumFields(); idx++)
            {
                ...
                if (destRecInfo.getFieldNum(name) == (unsigned) -1)
                {
                    // unmatched field
                    if ((field->flags & RFTMispayloadfield) == 0)
                        matchFlags |= match_payloadinkey;   // A keyed field is treated as non keyed (and ignored)
