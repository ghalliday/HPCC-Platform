...
bool CWsWorkunitsEx::onWUProcessGraph(IEspContext &context,IEspWUProcessGraphRequest &req, IEspWUProcessGraphResponse &resp)
{
    try
    {
        ...
        StringBuffer xml;
        Owned<IPropertyTree> xgmml = graph->getXGMMLTree(true, true, false); // merge in graph progress information
        toXML(xgmml.get(), xml);
    ...
    IConstWUGraph* graph, IArrayOf<IEspECLGraphEx>& graphs)
{
    ...
    Owned<IPropertyTree> xgmml = graph->getXGMMLTree(true, true, false);
