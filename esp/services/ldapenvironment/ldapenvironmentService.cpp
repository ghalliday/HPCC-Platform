...
#define MAX_USERNAME_LDAP_LEN 20
    ...
    while (true)
    {
        ...
        catch(...)
        {
            if (idx == 99)
            {
                notes.appendf("\nNon Fatal Error creating '%s'. Username too long, or already exists in target AD. Refer to LDAPEnvironment logfile for more information.", userName.str());
                return false;
            }
            ...
            else
            {
                ...
                itoa(++idx, szIndex, 10);
                tmpUserName.setLength(tmpUserName.length() - strlen(szIndex));
                tmpUserName.append(szIndex);
            }
