...
{
public:
    Owned<IFileIO> bufferedIO;      // MORE: This will go.
...
interface IDiskNodeCache : public IInterface
{
    ...
    virtual bool readNode(const CKeyIdAndPos & nodeId, CIndexReadCache & nodeData) = 0;
    // Insert a block of data into the disk node cache.  Depending on the configuration it may be multiple blocks
    virtual void wrtieNodes(const CKeyIdAndPos & nodeId, const CIndexReadCache & nodeData) = 0;
