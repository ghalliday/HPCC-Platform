...
//Unfortunate, but this needs to be vloned from jhtree because jlib cannot have a dependency on jhtree.
...
void EventRecorder::recordIndexLoad(unsigned fileid, offset_t offset, byte nodeKind, size32_t size, __uint64 elapsedTime, __uint64 readTime)
{
    ...
    {
        Owned<ISpan> span = createCompletedInternalSpan("IndexLoad", elapsedTime);
...
void EventRecorder::recordDaliEvent(EventType event, const char * path, __int64 id, stat_type elapsedNs, size32_t dataSize)
{
    ...
    if (unlikely(createSpans))
    {
        Owned<ISpan> span = createCompletedInternalSpan(queryEventName(event), elapsedNs);
