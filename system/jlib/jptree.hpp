...
enum PTreeVisitorAction
{
    PTVAcontinue,       // Continue with other possible matches
    PTVAstop,           // Stop visiting
};

interface IPropertyTreeVisitor : extends IInterface
{
    virtual PTreeVisitorAction visit(IPropertyTree & tree, const char * optAttribute) = 0;
};
...
interface jlib_decl IPropertyTree : extends serializable
{
    ...
    virtual PTreeVisitorAction visit(IPropertyTreeVisitor &visitor, const char *xpath = nullptr, IPTIteratorCodes flags = iptiter_null) const = 0;
