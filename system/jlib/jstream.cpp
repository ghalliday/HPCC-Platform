...
void CFileOutputStream::writeByte(byte b)
{
    if (_write(handle, &b, 1) != 1)
        throw MakeStringException(JLIBERR_SystemErrorWhileWritingByte0xXN, "Error while writing byte 0x%x\n", (unsigned)b);
}
...
void CFileOutputStream::writeBytes(const void *b, int len)
{
    ...
    if (written < 0)
        throw MakeStringException(JLIBERR_SystemErrorWhileWritingDBytesN, "Error while writing %d bytes\n", len);
    if (written != len)
        throw MakeStringException(JLIBERR_SystemTruncatedDWhileWritingDBytesN, "Truncated (%d) while writing %d bytes\n", (int)written, len);
}
