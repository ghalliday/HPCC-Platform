...
class TbbParallelMergeSorter
{
            ...
            loop
            {
                ...
                if (depth < sorter.parallelMergeDepth)
                {
                    ...
                    if (partitions > 1)
                    {
                        ...
                        {
                            MergeTask * mergeFwdTask = new (allocate_additional_child_of(*next)) MergeTask(sorter.compare, result, n1, src, n2, src+n1, n1);
                            mergeFwdTask->set_ref_count(1);
                            MergeTask * mergeRevTask = new (allocate_additional_child_of(*next)) MergeRevTask(sorter.compare, result, n1, src, n2, src+n1, n2);
    ...
    public:
        ...
        void adjustRange(unsigned deltaLeft, unsigned numLeft, unsigned deltaRight, unsigned numRight, unsigned num)
        {
            src1 += deltaLeft;
            n1 = numLeft;
            src2 += deltaRight;
    ...
    public:
        ...
        {
            posLeft = new unsigned[numPartitions+1];
            posRight = new unsigned[numPartitions+1];
        ...
        void calculatePartitions()
        {
            ...
            {
                for (unsigned skip=numPartitionSamples*2; skip-- != 0; )
                {
