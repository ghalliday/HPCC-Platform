...
void Allocator::_reallocate(aindex_t newLen, size32_t itemSize)
{
    ...
    if (newLen > ARRAY_CHUNK_DOUBLE_LIMIT)
    {
        ...
        if (newLen >= newMax) // wraparound
        {
            ...
            throw MakeStringException(JLIBERR_MemoryAllocation, "Out of memory (overflow) in Array allocator: itemSize = %u, trying to allocate %u items",itemSize, newLen);
        }
    }
    else
    {
    ...
    if (!newhead) 
    {
        ...
        throw MakeStringException(JLIBERR_MemoryAllocation, "Out of memory in Array allocator: itemSize = %u, trying to allocate %u items",itemSize, max);
    }
    max = newMax;
    _head = newhead;
};
...
void Allocator::_reallocateExact(aindex_t newLen, size32_t itemSize)
{
    ...
    if (used > newLen)
        throw MakeStringException(JLIBERR_MemoryAllocation, "Reallocate an array would contain too few items: itemSize = %u, trying to allocate %u items contains %u",itemSize, newLen, used);
