...
void StatisticsFilter::addFilter(const char * filter)
{
    ...
    else if (hasPrefix(filter, "value[", false))
    {
        ...
        throw MakeStringException(JLIBERR_UtilUnknownStatsFilterSExpectedCreatorCreatortypeDepthKindMeasureScopeScopetype, "Unknown stats filter '%s' - expected creator,creatortype,depth,kind,measure,scope,scopetype", filter);
}
