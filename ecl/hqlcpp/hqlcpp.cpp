...
unsigned HqlCppTranslator::getFilterDepthEstimate(BuildCtx & ctx, const HqlExprArray & conds)
{
...
void HqlCppTranslator::doBuildFilterToTarget(BuildCtx & ctx, const CHqlBoundTarget & isOk, HqlExprArray & conds, bool invert)
{
    ...
    if (getFilterDepthEstimate(ctx, conds) >= MAX_NESTED_CASES)
    {
        BuildCtx nestedctx(ctx);
        unsigned max = conds.ordinality();
        unsigned curIndex = 0;
        StringBuffer label;
        getUniqueId(label.append("l"));

        buildExprAssign(nestedctx, isOk, queryBoolExpr(invert));       // if(!invert,false,true) => invert
        for (unsigned outer =0; outer < max; outer++)
        {
            OwnedHqlExpr next = getInverse(&conds.item(outer));
            expandAliases(nestedctx, next, NULL);

            BuildCtx subctx(nestedctx);
            buildFilter(subctx, next);

            subctx.addGoto(label);
            nestedctx.addGroup();
        }
        buildExprAssign(nestedctx, isOk, queryBoolExpr(!invert));      // if(!invert,true,false) => !invert
        nestedctx.addLabel(label);
    }
    else
    {
        unsigned max = conds.ordinality();
...
void HqlCppTranslator::doBuildFilterAnd(BuildCtx & ctx, IHqlExpression * expr)
{
    ...
    unsigned depthEstimate = getFilterDepthEstimate(ctx, conds);
    unsigned numConds = conds.ordinality();
