...
void HqlCppTranslator::buildJoinMatchFunction(BuildCtx & ctx, const char * name, IHqlExpression * left, IHqlExpression * right, IHqlExpression * match, IHqlExpression * selSeq)
{
    if (match)
    {
        ...
        OwnedHqlExpr cseMatch = options.spotCSE ? spotScalarCSE(match, NULL, queryOptions().spotCseInIfDatasetConditions, queryOptions().optimizeInlineOperations) : LINK(match);
...
void KeyedJoinInfo::buildIndexReadMatch(BuildCtx & ctx)
{
    ...
    if (matchExpr)
    {
        ...
        if (translator.queryOptions().spotCSE)
            matchExpr.setown(spotScalarCSE(matchExpr, NULL, translator.queryOptions().spotCseInIfDatasetConditions, translator.queryOptions().optimizeInlineOperations));
