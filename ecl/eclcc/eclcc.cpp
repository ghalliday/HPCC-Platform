...
void EclCC::processDefinitions()
{
    ForEachItemIn(iDef, definitions)
    {
        ...
        repositoryManager.addSingleDefinitionEclRepository(module, attr, contents, true);
    }
}
...
void EclCC::processXmlFile(EclCompileInstance & instance, const char *archiveXML)
{
    ...
    EclRepositoryManager localRepositoryManager(repositoryManager); // Definitions, plugins, std library etc.
...
void EclCC::processFile(EclCompileInstance & instance)
{
    ...
    else
    {
        ...
        EclRepositoryManager localRepositoryManager(repositoryManager); // don't include -I
        if (!optNoBundles)
            localRepositoryManager.addQuerySourceFileEclRepository(&instance.queryErrorProcessor(), eclBundlePath.str(), 0, 0);
...
void EclCC::processReference(EclCompileInstance & instance, const char * queryAttributePath)
{
    ...
    EclRepositoryManager localRepositoryManager(repositoryManager);
    if (!optNoBundles)
