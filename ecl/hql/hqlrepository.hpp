...
public:
    ...
    explicit EclRepositoryManager(const EclRepositoryManager & other);
    ...
    IEclRepository * resolveDependentRepository(IIdAtom * name, const char * defaultUrl);
...
private:
    ...
    IArrayOf<IEclRepository> dependencies;
    IArrayOf<IEclRepository> sharedSources;     // plugins, std library, bundles
    IArrayOf<IEclRepository> allSources;        // also incldues -D options
