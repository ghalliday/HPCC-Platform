...
bool SortSlaveMP::marshall(ISortSlaveMP &slave, ICommunicator* comm, mptag_t tag)
{
    ...
    if (mb.length()==0) {
        LOG(MCthorDetailedDebugInfo, "Stopping SortSlaveMP::marshall");
        return false;
    }
    byte fn;
    mb.read(fn);
...
#ifdef FULLTRACE
    StringBuffer tmp1;
    LOG(MCthorDetailedDebugInfo, ">SortSlaveMP::marshall(%d) got %d from %s tag %d replytag %d",(int)fn, mb.length(), mb.getSender().getEndpointHostText(tmp1).str(),tag,mb.getReplyTag());
#endif
...
#ifdef FULLTRACE
        StringBuffer tmp1;
        LOG(MCthorDetailedDebugInfo, "<SortSlaveMP::marshall(%d) send %d to %s tag %d",(int)fn, mbout.length(), mbout.getSender().getEndpointHostText(tmp1).str(),mbout.getReplyTag());
#endif
        comm->reply(mbout);
    }
    if (err.get())
